...
1. 将代理写入 Codex 专用配置 `~/.codex/.env`，供新版 `codex.exe app-server`、模型请求和 WebSocket 读取：
...
`~/.codex/.env` 只由 Codex 加载，不是 Windows 用户级环境变量，不会让其它应用使用代理。脚本会保留该文件中的其它配置，只更新由 `proxy-codex` 管理的代理段。脚本不会修改系统代理或 WinHTTP 代理。
...
| 文件 | 作用 |
| --- | --- |
| `start-codex-with-clash-proxy.ps1` | 启动 ChatGPT/Codex，并写入 Codex 专用代理配置。 |
...
脚本会自动从 `AppxManifest.xml` 解析应用激活 ID，并通过 Windows 应用激活 API 启动，同时传入 `--proxy-server=http://127.0.0.1:7890`。这条路径不要求管理员权限，也不要求开启系统代理。

#### 更新后反复显示重连

新版桌面应用把主要网络请求放在独立的 `codex.exe app-server` 子进程中。仅给 ChatGPT/Chromium 主进程传 `--proxy-server` 时，app-server 不会自动继承该参数，可能出现流式响应反复重连、`Failed to send request` 或请求超时。

当前脚本会在重启应用前同步更新 `~/.codex/.env`，让 Chromium 和 app-server 两层都使用同一个代理。运行快捷方式或执行：
